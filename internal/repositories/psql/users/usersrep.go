...
func (r *UsersRepository) SaveUser(ctx context.Context, username string, password []byte) (int64, error) {
	...
	if err := row.Scan(&id); err != nil {
		...
		if pgErr, ok := err.(*pq.Error); ok && pgErr.Code.Name() == psql.CodeConstraintUnique {
			return 0, fmt.Errorf("%s, %w", op, ErrUserAlredyExists)
		}
...
func (r *UsersRepository) GetUser(ctx context.Context, username string) (models.User, error) {
...
func (r *UsersRepository) SetInactive(ctx context.Context, userId int64) error {
